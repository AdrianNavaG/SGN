...
//				$( "#labelModal" ).text( "hay expedientes" );
				...
				for (i=0; i< response.length; i++){
					...
					$('#tablecliente tr:last').after("<tr><td>" + response[i].idCliente +
					"</td>"+"<td>" + response[i].apellidoPaterno + "</td>"+
					"</td>"+"<td>" + response[i].apellidoMaterno + "</td>"+
					"</td>"+"<td>" + response[i].nombre + "</td>" +
					"</td>"+"<td>" + response[i].fechaAlta + "</td>" +
					"</td>"+"<td>" + response[i].fechaNacimiento + "</td>" +
					"</td>"+"<td>" + response[i].rfc + "</td>" +
					"<td></div>"+
					"<div class='form-group col-xs-10 col-sm-4 col-md-4 col-lg-4 text-right'>" +
						"<button type='button' class='btn btn-outline-primary btn-sm' onclick='selectclient(\"" + recursiveEncoded + "\")'>" +
...
function selectclient(cliente){
	...
	var clienteArray = cliente.split("&");
	var clienteArrayFinal = new Array();
	for (i = 0; i < clienteArray.length; i++) { 
	    var clienteArrayAux = clienteArray[i].split("=");
//	    console.log("val1: "+expedienteArrayAux[0]);
//	    console.log("val2: "+expedienteArrayAux[1]);
	    clienteArrayFinal[i]=clienteArrayAux[1];
	}
	
	alert(clienteArrayFinal[4].substring(0,10));
	alert(clienteArrayFinal[9]);
					...
					"<div class='card'>"+
						"<form action=''>"+
							...
							"<div class='card-header'>"+
								"<strong>Cliente: "+clienteArrayFinal[47].replace(/[+]/g,' ')+"</strong>"+
							"</div>"+
							...
							"<input type='text' id='id_cliente' name='id_cliente' value='"+clienteArrayFinal[0]+"'"+
							...
							"<label for='fecha_alta' class=' form-control-label'>Fecha "+
								"Alta</label> <input type='text' id='fecha_alta' value='"+clienteArrayFinal[4].substring(0,10)+"'"+
								"class='form-control'  readonly>"+
						    "</div>"+
							
							"<div class='form-group col-xs-10 col-sm-4 col-md-4 col-lg-4'>"+
							"<label for='apellido_pat' class=' form-control-label'>Apellido Paterno</label>"+
							"<input type='text' id='apellido_pat' name='apellido_pat' value='"+clienteArrayFinal[1].replace(/[+]/g,' ')+"'"+
							...
							"<input type='text' id='apellido_mat' name='apellido_mat'value='"+clienteArrayFinal[2].replace(/[+]/g,' ')+"'"+
								"class='form-control' placeholder='Ingresa Apellido Materno' >"+
						    "</div>"+
						    
							"<div class='form-group col-xs-10 col-sm-4 col-md-4 col-lg-4'>"+
							"<label for='nombre' class=' form-control-label'>Nombre(s)</label>"+
							"<input type='text' id='nombre' name='nombre' value='"+clienteArrayFinal[3].replace(/[+]/g,' ')+"'"+
							...
							"<input type='date' pattern='dd/mm/yyyy' id='fecha_nacimiento' value='"+clienteArrayFinal[5].substring(0,10)+"'"+
								" class='form-control'>"+
						    "</div>"+
